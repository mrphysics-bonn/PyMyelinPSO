...
class PSOpreparation_SI(PSOclass):
    ...
    def get_obs_data(self, _filter_bool: bool(), _filter_vals: tuple(), _signal=None, _data_type='invivo'):
        ...
        if self.config.source.signal.T2 == True:
            ...
            filt_data = np.empty_like(data_T2)
            
            if _filter_bool == True:
                from scipy.ndimage import gaussian_filter
                for i in range(24):
                    filt_data[:,:,i] = gaussian_filter(data_T2[:,:,i], _filter_vals)
                data_T2 = filt_data
        ...
        if self.config.source.signal.T2S == True:
            ...
            filt_data = np.empty_like(data_T2S)
            
            if _filter_bool == True:
                from scipy.ndimage import gaussian_filter
                for i in range(24):
                    filt_data[:,:,i] = gaussian_filter(data_T2S[:,:,i], _filter_vals)
                data_T2S = filt_data
        ...
        if self.config.source.signal.T1 == True:
            ...
            filt_data = np.empty_like(data_T1)
            
            if _filter_bool == True:
                from scipy.ndimage import gaussian_filter
                for i in range(0,24):
                    filt_data[:,:,i] = gaussian_filter(data_T1[:,:,i], _filter_vals)
                data_T1 = filt_data
