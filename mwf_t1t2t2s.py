"""
Myelin Water Fraction (MWF) analysis with NNLS on relaxation MRI data (T1,T2,T2star)
Tony Stoecker, DZNE Bonn, 2023
"""
...
if pyepg_found:
    ...
    print('warning: module pyepg not found ... try to continue with (slower) EPyG implementation')
...
def mwf_nnls(signal, T_grid, A, beta=0.0, T_thresh_lo=0.0, T_thresh_up=20.0):
    """
    estimate the MWF using NNLS.
    Input (signal, T_grid, A, beta, T_thresh_lo, T_thresh_up)
    signal      : shape (n,) MR signal (T1, T2, or T2*)
...
def T2_decay_pyepg(epg, step, T1, T2, te, tr, alpha, beta, etl):
    ...
    if step == 0:
        ...
        for n in range(sat_loops):
            epg.Step(alpha, 0, te/2)           # excitation
            for m in range(etl):             # ETL loop
                epg.Step(beta, 90, te/2)       # refocusing
                if m < etl-1:
                    epg.Step(0, 0, te/2)       # TE delay
                    if n == sat_loops-1:       # after T1-saturation ...
                        return epg.GetMagA()  # ... return signal amplitude after first echo! (i==0)
                else:
                    epg.Step(0, 0, tr-etl*te)  # TR delay
    # next step in echo train
    else:
        epg.Step(beta, 90, te/2)
        epg.Step(0, 0, te/2)
        return epg.GetMagA()


def T2_decay_EPyG_EchoTrain(epg, T2, params):
    te = params['TE']
    beta = params['beta']
    T1 = params['T1']
    tr = params['TR']
    ...
    alpha = params['alpha']
    # init EPG at first echo
    epg['T_ex'] = ops.Transform(alpha=np.deg2rad(
        alpha), phi=0.0, name="Excitation")  # Excitation operator
    epg['T_ref'] = ops.Transform(alpha=np.deg2rad(beta), phi=np.deg2rad(
        90.0), name="Refocusing")  # Refocusing operator
    epg['S'] = ops.Shift(shifts=1, autogrow=True,
                         name="Dephase")  # Shift operator
    epg['E'] = ops.Epsilon(TR_over_T1=te/(2*T1), TR_over_T2=te /
                           (2*T2), name="Relaxation")  # relaxation operator
    epg['O'] = ops.Observer(f_states=(0,), z_states=(
        0,), name="ADC")  # observer operator
    # Composite operator: (from right to left!)
    # shift, relaxation, refocussing, shift, relaxation, observation
    epg['C'] = ops.CompositeOperator(
        epg['O'], epg['E'], epg['S'], epg['T_ref'], epg['E'], epg['S'], name="mSE")
    # initialize EPG
    epg['EPG'] = epyg.EpyG(initial_size=256, m0=1.0)
    sat_loops = 4              # number of T1 saturation loops
    for n in range(sat_loops):  # outer loop to steady state (T1 saturation)
        epg['O'].clear()
        epg['T_ex'] * epg['EPG']
        for _ in range(etl):
            epg['C'] * epg['EPG']
        for _ in range(2*(int(tr//te)-etl)):
            epg['S'] * epg['EPG']
            epg['E'] * epg['EPG']
    return np.abs(epg['O'].get_f(0))


def compute_gaussian(x, sigma, mean, scale):
...
class signal_models():
    """
    Helper class to calculate the system matrix for T1, T2, and T2star signal models
    """

    def __init__(self, signal_type):
        self.signal_type = signal_type  # 'T1', 'T2', or 'T2S'
        # dictionary of signal model functions to compute system matrix at position (i,j)
        self.mod = {'T1': self.T1star_decay, 'T2': self.T2_decay,
                    'T2S': self.T2S_decay, 'CT2S': self.c_T2S_decay}
        self.tgs = []                # time grid signal
        self.tgm = []                # time grid model
        self.fgm = []                # frequency grid model
        if pyepg_found:              # prepare pyepg call
            self.epg = pyepg.PyEPG()
        else:                        # prepare EPyG call
            self.epg = {'T_ex': [], 'T_ref': [], 'S': [],
                        'E': [], 'O': [], 'C': [], 'EPG': []}
    ...
    def T1star_decay(self, i, j, params):
        """T1-star decay formula including inversion efficiency. See e.g.
        Deichmann, R., D. Hahn, and A. Haase. “Fast T1 Mapping on a Whole-Body Scanner.” 
        MRM 42, no. 1 (July 1999): 206–9. https://doi.org/10.1002/(SICI)1522-2594(199907)42:1<206::AID-MRM28>3.0.CO;2-Q.
        """
        t = self.tgs[i]
        T1 = self.tgm[j]
        ...
        TD = params["TD"]
        IE = params["IE"]
        T1s = 1/(1/T1-np.log(np.cos(alpha))/TR)         # T1-star
        # steady state magnetization (Ernst)
        mss = (1-np.exp(-TR/T1))/(1-np.exp(-TR/T1s))
        # magnetization after inversion
        mi = IE*(1 + (mss-1)*np.exp(-TD/T1))
        return mss - (mi+mss) * np.exp(-t / T1s)
    ...
    def T2_decay(self, i, j, params):
        ...
        etl = params['ETL']
        alpha = params['alpha']
        return T2_decay_pyepg(self.epg, step, T1, T2, te, tr, alpha, beta, etl)
    ...
    def system_matrix(self, t_grid_signal, t_grid_model, params, b1scale=0.0):
        ...
        for j in range(n):
            if self.signal_type == 'T2' and pyepg_found == False:
                A[:, j] = T2_decay_EPyG_EchoTrain(
                    self.epg, self.tgm[j], params)
            else:
...
class mwf_data():
    ...
    It is assumed, that 
    1. all nifti data are in the same voxel space: T1, T2, T2star, and B1 !
    2. the first three dimensions are [x,y,z], the fourth dimension of  T1 / T2, T2star is TI / TE (inversion / echo times)
    3. B1 is a map of scaling factors, suitable to scale nominal flip angles of the sequences
    """
    def __init__(self, data_dir, KW_T1, KW_T2, KW_T2S, KW_T2SP, KW_B1):
        self.data_dir = data_dir
        self.axis='None'
        self.slice_num=-1
        self.slice   = {'T1':[],'T2':[],'T2S':[],'CT2S':[],'B1':[]}
        self.ni_t1   = None
        self.ni_t2   = None
        self.ni_t2s  = None
        self.ni_ct2s = None
        ...
        if len(KW_T1)>0 and KW_T1 != 'NO_FILE':
            T1  = [s for s in files if KW_T1  in s] 
            if len(T1)>0:   self.ni_t1  = nib.load(os.path.join(data_dir,T1[0]))
            else: print('WARNING: could not find T1 data with keyword %s' % KW_T1)
    ...
    def __str__(self):
        ...
        names  = ['T1','T2','T2S','CT2S','B1']
        niftis = [self.ni_t1,self.ni_t2,self.ni_t2s,self.ni_ct2s,self.ni_b1]
        ...
        slice_shape  = {'T1':np.shape(self.slice['T1']),'T2':np.shape(self.slice['T2']),'T2S':np.shape(self.slice['T2S']),'CT2S':np.shape(self.slice['CT2S']),'B1':np.shape(self.slice['B1'])}
    ...
    def prep_data(self, axis, slice_num):
        ...
        Results will be stored as member dictionary "slice" with keys "T1", "T2", "T2S" (if corresponding data exist)
        """
        self.axis = axis
        ...
        TYPE  = ['T1','T2','T2S','CT2S']
        NIMG  = [self.ni_t1,self.ni_t2,self.ni_t2s,self.ni_ct2s,self.ni_b1]
        # get slice data of T1, T2, T2star (magn), and T2star (phase)
        for i in range(4):
                try:    
...
class mwf_analysis():
    """
    Class to perform MWF analysis on T1, T2, and/or T2star data.
    """
    def __init__(self, data_dir, KW_B1='NO_FILE', KW_T1='NO_FILE', KW_T2='NO_FILE', KW_T2S='NO_FILE', KW_T2SP='NO_FILE',T1_TI=[],T2S_TE=[], verbose=False):
        """
        ...
        KW_T1   :   keyword which matches the T1 nifti filename
        KW_T2   :   keyword which matches the T2 nifti filename
        KW_T2S  :   keyword which matches the T2star nifti filename
        T1_TI   :   name of npy file with 1D-array of TI times (len must match the 4-th dimension of T1 nifti)
        ...
        self.data  = mwf_data(data_dir=data_dir, KW_T1=KW_T1, KW_T2=KW_T2, KW_T2S=KW_T2S, KW_T2SP=KW_T2SP, KW_B1=KW_B1)
        self.filter= 0
        self.tsig  = {'T1':[],'T2':[],'T2S':[]}       # time points of the signal (TI or TE) - see below
        self.tvnm  = {'T1':'TI','T2':'TE','T2S':'TE'} # variable names for signal time points 
        self.dist  = {'T1':[],'T2':[],'T2S':[]}       # example mwf distributions in case of synthetic data
        #nnls result at specific position (from function plot)
        self.nnls  = {'T1':{'mwf':[],'sig':[],'fit':[],'weights':[]},'T2':{'mwf':[],'sig':[],'fit':[],'weights':[]},'T2S':{'mwf':[],'sig':[],'fit':[],'weights':[]}}
        #the following members will be defined from functions prep_{t1,t2,t2s}_model
        self.tmod  = {'T1':[],'T2':[],'T2S':[]}       # time points of the model  (T1, T2, or T2* grid)
        self.sm    = {'T1':[],'T2':[],'T2S':[]}       # system matrices
        self.params= {'T1':None,'T2':None,'T2S':None} # sequence parameters
        self.sig_n_pts = {'T1': [], 'T2': [], 'T2S': []}
        self.model_grid_step = {'T1': [], 'T2': [], 'T2S': []}

        # the following members will be defined in function mwf_map
        self.mwf = {'T1': [], 'T2': [], 'T2S': []}  # result MWF maps
        ...
        # get TI, TE times from npy files (TE only for T2S )
        if len(T1_TI) > 0:
            self.tsig['T1'] = np.load(os.path.join(data_dir, T1_TI))
        if len(T2S_TE) > 0:
    ...
    def __str__(self):
        ...
        signals = ['T1', 'T2', 'T2S']
        buf += '\n\n.tsig -> signal time points, i.e. TI or TE values for T1 or T2 and T2star signals. Format: (min,max; number)'
        ...
        buf += '\n\n.tmod -> model time points for T1, T2 and T2star. Format: (min,max; number)'
        for i in range(len(signals)):
        ...
        sm_shape = {'T1': np.shape(self.sm['T1']), 'T2': np.shape(
            self.sm['T2']), 'T2S': np.shape(self.sm['T2S'])}
        mwf_shape = {'T1': np.shape(self.mwf['T1']), 'T2': np.shape(
            self.mwf['T2']), 'T2S': np.shape(self.mwf['T2S'])}
    ...
    def prep_data(self, axis, slice_num, signal_type, thresh=10.0, filter=0.0, N_b1_grid=50, verbose=False):
        """
        Prepare single slice to perform MWF analysis
        Input
        axis        :   'X', 'Y', or 'Z'
        slice_num   :   slice number   
        signal_type :   'T1','T2', or 'T2S'
        ...
        data = np.copy(self.data.slice[signal_type])
        
        tax = self.tsig[signal_type]
        ...
        if filter > 0.0:
            self.filter = filter                
            data = gaussian_filter(data, (filter, filter, 0))            
            self.data.slice[signal_type] = np.copy(data)
        
        # compute mask from first image of current contrast
        msk = np.abs(data[:, :, 0])
        mx = np.max(msk)
        ...
        # TMP TMP: if b1-map has large values, then scale it by assuming 60deg 3DREAM data
        if (np.max(self.data.slice['B1']) > 100.0):
            self.data.slice['B1'] = self.data.slice['B1']/600
        # range of B1 scalings in mask
    ...
    def prep_synthetic_data(self, axis, slice_num,  signal_type, mwf_thresh=0.025, SNR=500.0, seed=0, nprocs=8, dmean=[], dstdv=[], x=0, y=0, verbose=True):
        # default values: mean and standard deviation of Gaussian peaks for synthetic signals
        dist_mean = {'T1': [400, 1000], 'T2': [20, 80], 'T2S': [10, 40]}
        dist_stdv = {'T1': [100, 100], 'T2': [10, 10], 'T2S':  [8, 8]}
        if len(dmean) == 2:
        ...
        a = slice(None, None)
        # => x[:,:,slice_num] = x[a,a,s] or x[:,slice_num,:] = x[a,s,a]
        ...
        def distribution(mwf):
            ...
            return mwf*np.exp(-(m[0]-tm)**2/(2*s[0]**2))+(1-mwf)*np.exp(-(m[1]-tm)**2/(2*s[1]**2))
        # store example distribution for mwf[x,y]
        if x == 0:
            x = n1//2
        if y == 0:
            y = n2//2
    ...
    def prep_t1_model(self, tr=4.0, alpha=4.0, td=1000.0, ie=0.95,  T1min=10, T1max=3500, nT1=350, verbose=True):
        """
        prepare T1 model and compute system cube
        """
        # Look-Locker signal model and sequence parameters
        # beta only for compatibility with T2 model
        params = {'TR': tr, 'alpha': alpha, 'TD': td, 'IE': ie, "beta": 0.0}
        self.params['T1'] = params
        self.tmod['T1'] = np.linspace(T1min, T1max, nT1)
        # compute system matrix
        self.system_cube(signal_type='T1', params=params, verbose=verbose)

    def prep_t2_model(self, te=6.6, tr=900, etl=24, alpha=70, beta=180, T2min=1, T2max=300, nT2=300, T1=1000, verbose=True):
        """
        prepare T2 model and compute system cube
        """
    ...
    def prep_t2s_model(self, T2Smin=1, T2Smax=200, nT2S=200, verbose=True):
        ...
        # no seq params needed for T2* /  alpha, beta for compatibility with T1,T2 model
    ...
    def mwf_map(self, signal_type, mwf_thresh=(50, 500), nnls_beta=0.0, DS=1, nprocs=8, verbose=True):
        """
        ...
        self.mwf[signal_type], self.usc[signal_type] = self.mwf_map_nnls(signal_type=signal_type, mwf_thresh=mwf_thresh, nnls_beta=nnls_beta, DS=DS,
                                                                         nprocs=nprocs, verbose=verbose)

    def system_cube(self, signal_type, params, verbose):
        # compute system cube for T1, T2, or T2*, i.e. system matrices for all b1 scaling factors (called by prep_{t1,t2,t2s}_model)
        if verbose:
        ...
        A = np.zeros((len(t_grid_signal), len(
            t_grid_model), len(self.b1_grid)))
        progress = np.zeros((1,))
    ...
    def compute_decay_curve(self, model, matrix, signal_type,
        ...
        if model_component == "Gaussian":
            fast_component = compute_gaussian(
                self.tmod[signal_type], model[1], model[0],
                model[4]*model[5]/(1-model[5]))
            slow_component = compute_gaussian(
        ...
        if T2S_complex:
            ...
            slow_decay = np.multiply(
                ...
                dtype=np.complex128)
            decay_curve = np.add(fast_decay, slow_decay)
            decay_curve *= np.exp(-1j*model[8])
    ...
    def mwf_map_nnls(self, signal_type, mwf_thresh, nnls_beta, DS, nprocs, verbose):
        # calculate MWF in one slice for T1, T2, or T2* using NNLS (called by mwf_map)
        ...
        def inner_loop(i):
            for j in range(n2):
                if self.data.msk[DS*i, DS*j] == 1:
                    ...
                    mwf[i, j], usc[i, j], _ = mwf_nnls(
                        signal, t_grid, A[:, :, n_fs], nnls_beta, t_thresh_lo, t_thresh_up)
    ...
    def plot(self, signal_type, component='SUM', vmax=0.5, x=0, y=0, alpha=0.7, DS=1, save=False, verbose=True):
        """
        ...
        mwf_xy, dmc_xy, weights = mwf_nnls(signal, self.tmod[signal_type], A,
        ...
        fig.set_facecolor('w')
        if save:
            plt.savefig('mwf_plot_%s_%s%d_beta%5.4f.png' % (
                signal_type, self.data.axis, self.data.slice_num, self.nnls_beta))
