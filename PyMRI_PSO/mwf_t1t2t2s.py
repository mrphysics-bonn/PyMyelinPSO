...
# for PSO on windows-based systems --> pymp produces errors, because its not supported
import sys
...
class mwf_analysis():
    ...
    def __init__(self, data_dir, KW_B1='NO_FILE', KW_T1='NO_FILE', KW_T2='NO_FILE', KW_T2S='NO_FILE', KW_T2SP='NO_FILE',T1_TI=[],T2S_TE=[], verbose=False):
        ...
        self.filter= 0
        self.tsig  = {'T1':[],'T2':[],'T2S':[],'CT2S':[]}       # time points of the signal (TI or TE) - see below
        ...
        self.tmod  = {'T1':[],'T2':[],'T2S':[]}       # time points of the model  (T1, T2, or T2* system grid)
    ...
    def prep_data(self, axis, slice_num, signal_type, thresh=10.0, 
        ...
        # Define b1_grid only for T2 to avoid changes from T2S values in JI applications
    ...
    def system_cube(self, signal_type, params, verbose):
        '''
        System matrix for T1, T2, or T2* signal. 
        In case of T2, a system cube is being created
        --> based on all b1 scaling factors (called by prep_{t1,t2,t2s}_model)
