...
def mask_shape_combo(binary_array: np.array, comparison_array: np.array, i=0, transparency=0.5):
    ...
    # Rechteck lÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶scht Maske (setzt nur auf 0)
...
def mask_shape_rectangle(binary_array: np.array, comparison_array: np.array, i=0, transparency=0.5):
    ...
    def onselect(eclick, erelease):
        ...
        # Bereich auswÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤hlen und auf 0 setzen
...
def troubleshooting(config):
    ...
    if not SI and not JI:
        sys.exit('ATTENTION: DidnÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â´t you forget something? Both inversion types are still set as false.')
...
def compute_gauss_full(x, sigma, mean, scale):
    ...
    *args:
        ...
        sigma: np.array
            standard deviation / width (ÃƒÆ’Ã‚ÂÃƒâ€ Ã¢â‚¬â„¢)
        mean: np.array
            center of the Gaussian (ÃƒÆ’Ã…Â½Ãƒâ€šÃ‚Â¼)
        scale: np.array
