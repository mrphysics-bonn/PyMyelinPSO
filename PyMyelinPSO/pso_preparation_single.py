# SPDX-FileCopyrightText: 2026 Helmholtz-Zentrum fŸr Umweltforschung GmbH - UFZ
...
class PSOpreparation_SI(PSOclass):
    ...
    def handle_mask(self):
        ...
        if self.mask_use == False:
            self.masks = np.ones(self.obs_data[self.decay_types[0]].shape[:3], dtype=int)
            
        elif self.mask_use == True:
    ...
    def access_mwf_analysis(self):
        ...
        Find the source code of MWF analysis class object: mwf_t1t2t2s.py
        """
          
        if self.data_type == 'atlas':
            T2S_TE = self.config.source.file.TE if self.inv_T2S else []
    ...
    def calc_system_cube(self, slice_num: int):
        ...
        if self.data_type == 'atlas':
            ...
            for sig in self.decay_types:
                ...
                if self.mask_use == False:
                    self.masks[:, :, slice_num] = self.root_mwf.data.msk
        ...
        if self.inv_T2 == True:
            ...
            if self.data_type == 'invivo':
                ...
                self.root_mwf.prep_t2_model(te      = 6.6,                    # 6.6  | sego: 6.0
    ...
    def get_atlas_data(self):        
        ...
        atlas_path = os.path.join(self.data_dir, self.config.source.file.ATLAS)
        mwf_mean   = nib.load(atlas_path)
        
        return mwf_mean.get_fdata()
    
    def get_obs_data(self, _signal=None, _data_type='invivo'):
    ...
    def calculate_sys_param(self, slice_calc: list):
        ...
        if self.data_type == 'invivo':
            ...
            for i in range(slice_calc[0], slice_calc[-1]):
                ...
                self.sys_param[f'{i:02}'] = {f'{sig}_MATRIX': self.root_mwf.sm[sig],
                                             'B1_DATA':       self.root_mwf.data.slice['B1'],
                                             'B1_GRID':       self.root_mwf.b1_grid,
                                             f'{sig}_MASK':   self.masks[:,:,i],
                                             'CT2S_TE':       self.root_mwf.tsig['T2S'],
                                             'MeasData':      {self.decay_types[0]: self.obs_data[self.decay_types[0]][:,:,i]}}
        ...
        if self.data_type == 'atlas':
            ...
            for i in range(slice_calc[0], slice_calc[-1]):
                ...
                if self.config.source.data.norm_max == True and cpx == False:
                    max_val = np.max(self.root_mwf.data.slice[sig], axis=2, keepdims=True)
                    max_val[max_val == 0] = 1
                    self.obs_data[sig]    = self.root_mwf.data.slice[sig]/max_val
                
                elif self.config.source.data.norm_max == True and cpx == True:
                    max_val = np.max(np.abs(self.root_mwf.data.slice['T2S']), axis=2, keepdims=True)
                    max_val[max_val == 0] = 1
