...
class PSOpreparation_JI(PSOclass):
    ...
    def handle_mask(self):
        ...
        if self.mask_use == False:
            self.masks   = {sig: np.ones_like(self.obs_data[sig],dtype=int) for sig in self.signType}
            
        if self.mask_use == True:
            mask_suffix    = {sig: Path(self.mask_path[sig]).suffixes[0] for sig in self.signType}
            self.mask_path = {sig: os.path.join(self.proj_dir, self.mask_path[sig]) for sig in self.signType}
            self.masks     = {sig: [] for sig in self.signType}
    ...
    def calc_system_cube(self, slice_num):
        ...
        if self.data_type == 'invivo':
            ...
            self.b1_grid = {sig: [] for sig in self.signType}
            ...
            for sig in self.signType:
                ...
                self.b1_grid[sig] = self.rootMWF.b1_grid if sig == 'T2' else np.zeros((1,))
    ...
    def get_obs_data(self, _filter_bool: bool(), _filter_vals: tuple(), _data_type='invivo'):
    ...
    def calculate_sys_param(self, slice_calc: list):
        ...
        if self.data_type == 'invivo':
            ...
            for i in range(slice_calc[0], slice_calc[-1]):
                ...
                self.sys_param[f'{i:02}'] = {**{f'{sig}_MATRIX':  self.rootMWF.sm[sig]   for sig in self.signType},
                                             **{f'{sig}_MASK':    self.masks[sig][:,:,i] for sig in self.signType},
                                             **{f'{sig}_B1_GRID': self.b1_grid[sig]      for sig in self.signType},
                                             'B1_DATA':           self.rootMWF.data.slice['B1'],
                                             'CT2S_TE':           self.rootMWF.tsig['T2S'],
                                             'MeasData':          {sig: self.obs_data[sig][:,:,i] for sig in self.signType}}
            ...
            for i in range(slice_calc[0], slice_calc[-1]):
                ...
                if self.invT2 == True:
                    self.masks['T2']  = self.rootMWF.prep_synthetic_data(
                                             ...
                                             SNR  = SNR, seed = 4*SNR,  dmean = np.array([20,70]), dstdv=np.array([0.5,0.5]),
                ...
                if self.invT2 == True:
                    self.masks['T2S'] = self.rootMWF.prep_synthetic_data(
                                             ...
                                             SNR  = SNR, seed = 4*SNR,  dmean = np.array([15,60]), dstdv=np.array([0.5,0.5]), 
                ...
                for sig in self.signType:                        
                    ...
                    if sig == 'T2S' and self.config.source.data.norm_max == True and cpx == True:
                    ...
                    if sig != 'T2S' and self.config.source.data.norm_max == True and cpx == True:
                        max_val = np.max(self.rootMWF.data.slice[sig], axis=2, keepdims=True)
                        max_val[max_val == 0] = 1
                        self.obs_data[sig] = self.obs_data[sig]/max_val
                        continue
        
                    if self.config.source.data.norm_max == False and cpx == False:
                        self.obs_data[sig] = self.rootMWF.data.slice[sig]
                     
                    
                self.sys_param[f'{i:02}'] = {**{f'{sig}_MATRIX':  self.rootMWF.sm[sig]   for sig in self.signType},
                                             **{f'{sig}_MASK':    self.masks[sig][:,:,0] for sig in self.signType},
                                             **{f'{sig}_B1_GRID': self.rootMWF.b1_grid   for sig in self.signType},
                                             'B1_DATA':           self.rootMWF.data.slice['B1'],
                                             'CT2S_TE':           self.rootMWF.tsig['T2S'],
                                             'MeasData':          {sig: self.obs_data[sig] for sig in self.signType}}
